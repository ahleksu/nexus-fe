...
import { PerformanceFeedbackButton } from "@/components/performance-feedback-button";
...
const generateScoreData = () => {
  ...
  for (let i = 29; i >= 0; i--) {
    ...
    data.push({ date: formattedDate, internalQaScore, csatScore });
  }
  return data;
};
...
const scoreChartConfig = {
  internalQaScore: { label: "Internal QA Score", color: "oklch(0.5 0.25 280)" },
  csatScore: { label: "CSAT Score", color: "oklch(0.6 0.2 260)" },
} satisfies ChartConfig;

const sentimentChartConfig = {
  Positive: { label: "Positive", color: "oklch(0.5 0.25 280)" },
  Neutral: { label: "Neutral", color: "oklch(0.6 0.2 260)" },
  Negative: { label: "Negative", color: "oklch(0.7 0.15 240)" },
} satisfies ChartConfig;
...
export function AgentKpiDashboard({ agent, onBack }: AgentKpiDashboardProps) {
  const [range, setRange] = useState<DateRange | undefined>({
    from: new Date(2025, 3, 17),
    to: new Date(2025, 4, 17),
  });

  const scoreData = useMemo(() => generateScoreData(), []);

  // Pass scoreData to kpiData for Gemini prompt
  const kpiData = {
    totalInteractions: 182,
    avgHandleTime: "5m 42s",
    avgQaScore: "94/100",
    avgCsatScore: "4.7/5",
    fcrRate: "85%",
    aiSummariesUtilized: 160,
    docSearches: 110,
    aiSuggestionsAccepted: 98,
    sentimentData,
    recentInteractions,
    scoreData, // <-- include chart data
  };
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        {/* Performance Feedback Button */}
        <PerformanceFeedbackButton
          agent={agent}
          kpiData={kpiData}
                ...
                >
                  ...
                  >
                    {sentimentData.map((entry) => (
                      <Cell
                        key={entry.sentiment}
                        fill={`var(--color-${entry.sentiment})`}
                      />
                    ))}
                  </Pie>
                  <ChartTooltip
                    cursor={false}
                    content={<ChartTooltipContent nameKey="sentiment" />}
                  />
                  <ChartLegend
